...
const _getGridTransposeChartOptions = (questionData: any, chartData: any) => {
  ...
  questionData.subGroups.forEach((scale: any, index: number) => {
    newScale.push({
  ...
  questionData.scale.forEach((scale: any, index: number) => {
    newSubGroup.push({
      qId: scale.labelCode,
      labelText: scale.labelText,
  ...
  // const legends = questionData.subGroups;
  // const scales = questionData.scale;
  ...
  for (let i = 0; i < newScale.length; i++) {
    ...
    // debugger;
    const data = [];
    const legend = newScale[i];

    for (let j = 0; j < newSubGroup.length; j++) {
      const scale = newSubGroup[j];
      const optionData = getmatchedFind(
        newChartData[i].options[i].option,
        "_id",
        legend.labelCode
      );

      const label: any = getMatchedfilter(
        optionData.options,
        "option",
        scale.labelCode
      );

      const baseCount = _.sumBy(optionData.options, function (o: any) {
        return o.count;
      });
    ...
    series.push({
      name: legend.labelText,
      color: colorArr[i < colorArr.length ? i : 0],
      data,
      dataLabels,
    });
  }
  ...
  //   optionIndex++
  // ) {
  //   //console.log(questionData?.scale[optionIndex]?.labelText);

  //   const subGroup = questionData?.subGroups[optionIndex];
  ...
  //     data: [
  //       {
  //         name: questionData?.scale[optionIndex - 1]?.labelText,
  //         y: 10,
  //         baseCount: 5900,
  //       },
  //     ],
